...
const Reported = () => {
  ...
  const { data: reported, isPending } = useQuery({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {reported?.map((item, idx) => {
        return (
          <div key={idx} className="shadow p-5 rounded-md ">
            {/* <h1 className="text-2xl font-semibold text-center">
              Lorem ipsum dolor sit amet. dolor sit amet.
            </h1> */}
            <div className="mt-5 space-y-2 text-lg">
              <h1 className=" flex  items-center gap-1">
                <span className="font-medium flex items-center gap-1">
                  <MdOutlineReport className="text-blue-600 w-6 h-6" /> Report
                  by:
                </span>{" "}
                {item.emailBlock}
              </h1>
            ...
            <p className="italic font-semibold border border-red-600 text-red-600 py-3 px-5 w-full rounded-md text-center mt-4">
              <span className="">Feedback:</span> {item.feed}
            </p>
            <form className="flex items-center gap-3 mt-4">
              <div className="relative h-fit  border rounded-md w-full ">
                <select
